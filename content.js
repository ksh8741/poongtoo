(function () {
  ...
  function isThisOrLastMonth(ym) {
    if (!/^\d{4}\.\d{2}$/.test(ym)) return false;     // 형식 검사
    ...
    // 이번달
    if (y === thisY && m === thisM) return true;
  ...
  function sortedKeys(obj) {
    return Object.keys(obj)
      .sort((a, b) => {
        const monthRe = /^\d{4}\.\s?\d{2}$/;
        const isDayA = !monthRe.test(a);
        const isDayB = !monthRe.test(b);
  
        if (isDayA !== isDayB) return isDayA ? -1 : 1;
  ...
  async function renderPoongUI(allData, selectedDate) {
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width:110px;">최다 선물 BJ</th>
            </tr>
          </thead>
    ...
    const sortedDateList = sortedKeys(allData);
    ...
    if (allData['__merged__']) {
      ...
      mergedOption.value = '__merged__';
      mergedOption.textContent = '데이터 합산';
      if (selectedDate === '__merged__') mergedOption.selected = true;
      dateSelect.appendChild(mergedOption);
    }
        
    sortedDateList.forEach(k => {
      if (k === '__merged__') return; // 이미 위에서 처리함
      const opt = document.createElement('option');
      opt.value = k;
      opt.textContent = (k.length === 7) ? `${k.slice(0, 5)} ${k.slice(5)}` : k;
      if (k === selectedDate) opt.selected = true;
      dateSelect.appendChild(opt);
    });
    
    // ✅ 이 아래에 넣어줘 (덮어쓰기 안되도록 기존 onchange는 삭제)
    dateSelect.addEventListener('change', e => renderPoongUI(allData, e.target.value));

    const refresh = async () => {
      await clickMoreUntilDone();
    ...
    document.getElementById('erase-btn').onclick = () => {
      chrome.storage.local.get([STORAGE_KEY], result => {
        ...
        chrome.storage.local.set({ [STORAGE_KEY]: store }, () => {
          const nextDate = filterAndSortKeys(store)[0] || '';
          selectedDate = nextDate; // 🔥 꼭 갱신해야 이후 삭제 작동함
    ...
    document.querySelector('#export-btn').onclick = async () => {
      ...
      // 날짜 정규식 확인 (YYYY. MM. DD 또는 YYYY. MM)
      const isValidDate = date => /^\d{4}\.\s*\d{2}(\.\s*\d{2})?$/.test(date);
    
      for (const origKey of keys) {
        if (!isValidDate(origKey)) continue;
    ...
    document.querySelector('#import-btn').onclick = () => {
      ...
      input.webkitdirectory = true;  // ★ 폴더 통째로 선택 가능
    
      input.onchange = async e => {
        const files = Array.from(e.target.files)
          .filter(f => /\.json$/i.test(f.name));          // JSON만
        if (!files.length) return;
    
        // ① 기존 저장 데이터
        const existing = await new Promise(r =>
          chrome.storage.local.get('poong_donation_by_date',
            res => r(res.poong_donation_by_date || {}))
        );
        ...
        for (const file of files) {
          try {
            ...
            // 폴더/공백 제거 → 'YYYY.MM[.DD]' 형태만 남김
            const key = file.webkitRelativePath        // 폴더째 선택 시
              ? file.webkitRelativePath.split('/').pop().replace(/\.json$/i,'')
              : file.name.replace(/\.json$/i,'').trim();
    
            existing[key] = data;                      // 덮어쓰거나 추가
          } catch (err) {
        ...
        chrome.storage.local.set({ poong_donation_by_date: existing }, () => {
          const latest = Object.keys(existing).sort().pop() || '';
          renderPoongUI(existing, latest);             // 새로고침 없이 갱신
  ...
  window.showPoongUI = async function () {
    chrome.storage.local.get(STORAGE_KEY, (result) => {
      ...
      const sorted = sortedKeys(saved);
      const latest = sorted[0] || '';
      renderPoongUI(saved, latest);
    });
  };
